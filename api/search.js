...
function loadServerData() {
    ...
    try {
        ...
        // notionData.js 로드
        const notionPath = path.join(process.cwd(), 'notionData.js');
        if (fs.existsSync(notionPath)) {
            const notionContent = fs.readFileSync(notionPath, 'utf-8');
            // NOTION_DATA 배열 추출
            const notionMatch = notionContent.match(/const NOTION_DATA = (\[[\s\S]*?\]);/);
            if (notionMatch) {
                const notionData = eval(notionMatch[1]);
                allData = allData.concat(notionData);
                console.log(`📘 Notion 데이터 로드: ${notionData.length}개`);
            }
        }

        // qaData.js 로드
        const qaPath = path.join(process.cwd(), 'qaData.js');
        if (fs.existsSync(qaPath)) {
            const qaContent = fs.readFileSync(qaPath, 'utf-8');
            // QA_DATA 배열 추출
            const qaMatch = qaContent.match(/const QA_DATA = (\[[\s\S]*?\]);/);
            if (qaMatch) {
                const qaData = eval(qaMatch[1]);
                allData = allData.concat(qaData);
                console.log(`📊 QA 데이터 로드: ${qaData.length}개`);
            }
        }
...
function calculateSpecialtyBonus(item, userSpecialty) {
    ...
    if (specialties && specialties.includes(userSpecialty.code.toLowerCase())) {
        ...
        bonus += Math.min(matchCount * 0.2, 0.8);
    }
...
function smartSearch(data, queryPlan, maxResults = 10, userSpecialty = null) {
    ...
    let results = candidates.map(item => {
        ...
        // 의도 기반 가중치
        const isHowToIntent = intent === '정보요청' || intent === '절차안내';
