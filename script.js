...
async function getBotResponse(userMessage) {
    ...
    try {
        ...
        if (result.text.includes('[OFF_TOPIC]')) {
            ...
            // Rambling 방지: 데이터가 없다면서 답변을 길게(150자 이상) 하는 경우, 
            // 뒤쪽의 사과 문구만 추출하거나 전체를 깔끔하게 정리
            if (cleanText.length > 150) {
                console.log('⚠️ [NO_DATA] 태그와 함께 불필요한 긴 설명이 감지되어 정리합니다.');
                // "죄송합니다" 이후의 문구만 살리거나, 너무 길면 기본 메시지 사용
                const apologyIndex = cleanText.lastIndexOf('죄송합니다');
                if (apologyIndex !== -1) {
                    cleanText = cleanText.substring(apologyIndex);
                }
            }
            // 인용 번호 제거 (데이터가 없다면 인용도 의미 없음)
            cleanText = cleanText.replace(/\[\d+\]/g, '').trim();
